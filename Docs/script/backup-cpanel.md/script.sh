        ...
        if [[ $ID_user -ge 1000 ]];
            ...
            then
                ...
                then
                    ...
                    zip -rq /tmp/bk_db/file $src_Code > /dev/null 2>&1
                    size=$(du -b /tmp/bk_db/file.zip | awk '{print $1}')
                    mv -i /tmp/bk_db/*.zip /$src_folder/$Username/"code-$(date "+%H%M%d%m%Y")".zip 
...
f_backup_db(){
    ...
    do
        ...
        then
            ...
            mysqldump -u root $dbname > /tmp/bk_db/file.sql
